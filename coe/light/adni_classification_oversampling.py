...
def parse_args():
    ...
    parser.add_argument("--exp", default='ADNI_MCI_CN_cls_nc_mci_ukbiobank_braindynamo_mask', help="Adjusted in code: Experiment foler name")
...
def main():
    ...
    # all_files, label_map_str = load_adni_file_list(
    #     '/mnt/sourav/fresh_ADNI/dtseries',
    #     '/mnt/sourav/fresh_ADNI/resting_state_fmri_info_sex_new.csv',
    #     'researchGroup'
    # )

    all_files, label_map_str = load_adni_amyloid_file_list(
		'/mnt/sourav/fresh_ADNI/dtseries',
		'/mnt/sourav/fresh_ADNI/amyloid_filtered_with_path.csv',
	)

    # logger.info("Total files loaded: %d", len(all_files))
    # # ─── Build a mapping from researchGroup‐string → integer class index ──────────
    # # unique_groups = { grp for (grp, age, sex) in label_map_str.values() }
    # # group2idx = { grp: idx for idx, grp in enumerate(sorted(unique_groups)) }
    # group2idx = {'CN': 0, 'MCI': 1}
    # # group2idx = {'CN': 0, 'AD': 1}
    # # group2idx = {'F': 0, 'M': 1}
    # # group2idx = {'positive': 1, 'negative': 0}
    # logger.info("Found %d unique groups: %s", len(group2idx), group2idx)
    # pos_files = [f for f, (lab, _, _) in label_map_str.items() if lab == 'MCI']
    # neg_files = [f for f, (lab, _, _) in label_map_str.items() if lab == 'CN']
    # # n = min(len(pos_files), len(neg_files))
    # n_to_augment = len(neg_files) - len(pos_files) 
    # random.seed(args.seed)           # for reproducibility
    # # pos_sample = random.sample(pos_files, n)
    # pos_augment_files = random.choices(pos_files, k = n_to_augment)
    # balanced_files = pos_files + neg_files + pos_augment_files
    # random.shuffle(balanced_files)
    # all_files = balanced_files
    # # logger.info("Balanced classes to %d positives and %d negatives (total %d files)",
    # # 			n, n, len(all_files))
    # logger.info("Balanced classes to %d positives and %d negatives (total %d files)",
    #             len(pos_files + pos_augment_files), len(neg_files), len(all_files))
    
    # ─── Convert label_map_str (rel_path→(grp_str, age)) into label_map_idx (rel_path→(grp_idx, age)) ─
    label_map_idx = {}
    for rel_path, (age, sex) in label_map_str.items():
        # If some rel_path had a group not in group2idx, you'd catch it here.
        # grp_idx = group2idx[grp_str]
        label_map_idx[rel_path] = (age, sex)
    labels    = [label_map_idx[f][0] for f in all_files]   # 0 for CN, 1 for any MCI

    # label_map_idx = {}
    # for rel_path, (label, amyloid_val) in label_map_str.items():
    # 	label_map_idx[rel_path] = (label, amyloid_val)
    # # pdb.set_trace()
    # pos_files = [f for f, (lab, amyloid_val) in label_map_str.items() if lab == 1]
    # neg_files = [f for f, (lab, amyloid_val) in label_map_str.items() if lab == 0]

    # # 2) Decide your target size (min of the two)
    # n = min(len(pos_files), len(neg_files))

    # # 3a) Under-sample the larger class
    # random.seed(args.seed)           # for reproducibility
    # pos_sample = random.sample(pos_files, n)
    # neg_sample = random.sample(neg_files, n)

    # # 4) Rebuild & shuffle the balanced list
    # balanced_files = pos_sample + neg_sample
    # random.shuffle(balanced_files)

    # # 5) Replace all_files & labels with the balanced version
    # all_files = balanced_files
    # labels    = [label_map_idx[f][0] for f in all_files]
    ...
    train_dataset = ADNI_Dataset(
        data_dir       = '/mnt/sourav/fresh_ADNI/dtseries',
        file_list      = train_files,
        label_map      = label_map_idx,         # now maps rel_path→(grp_idx, age)
        ...
        label_column   = 'researchGroup'        # <-- this makes __getitem__ return grp_idx
    )
    ...
    for epoch in range(args.epoch):
        ...
        for x, coeffs, y_true, time in tqdm(train_dl, leave=False):
            ...
            # features = torch.mean(U, dim=1)          			# take average along time
            features = U[:, -1, :]        
        ...
        with torch.no_grad():
            ...
            y_pred_all = torch.cat(y_pred_all, dim=0)  # shape: [N, 2] for binary classification
            # --- compute ROC points and AUC ---
            y_true_np = np.array(test_trues)
            # get predicted probabilities for class=1
            y_prob_np = F.softmax(torch.tensor(y_pred_all), dim=1).numpy()[:, 1]
            # --- plot and save ROC curve ---
            plt.figure(figsize=(6, 6))
            def bootstrap_mean_roc(y_true, y_score, n_bootstraps=2000, seed=0, n_grid=400, smooth=True):
                ...
                for _ in range(n_bootstraps):
                    ...
                    fpr, tpr, _ = roc_curve(y_true[idx], y_score[idx], drop_intermediate=False)
                    ...
                    # 2) interpolate with explicit clamping at ends
                ...
                # 3) optional visual smoothing that won't lift the edge
                if smooth:
                    from scipy.ndimage import gaussian_filter1d
                    mean_tpr = gaussian_filter1d(mean_tpr, sigma=2, mode="nearest")
                    # re-pin the ends after smoothing
                    mean_tpr[0] = 0.0
                    mean_tpr[-1] = 1.0

                # sanity check
                # print("grid[0], mean_tpr[0] =", grid[0], mean_tpr[0])

                return grid, mean_tpr, std_tpr, mean_auc
            grid, mean_tpr, std_tpr, mean_auc = bootstrap_mean_roc(y_true_np, y_prob_np)
            plt.plot(grid, mean_tpr, lw=2, label=f"Mean ROC (AUC={mean_auc:.3f})")
            ...
            roc_dir = os.path.join(args.exp, "roc_curve")
            os.makedirs(roc_dir, exist_ok=True)


            plt.savefig(f"{args.exp}/roc_curve/roc_epoch_{epoch}.png", dpi=300)
            plt.close()
