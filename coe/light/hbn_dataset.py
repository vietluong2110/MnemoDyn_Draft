...
class HBNDataModule:
    ...
    def setup(self):
        ...
        self.test_dataset = HBN_Dataset(self.data_dir, self.test_files, self.label_map, self.time_step, 
                                    self.interpol_method, self.one_channel, self.subset, self.dim_D, 
                                    self.target_length, self.normalizer, self.num_parcels, self.label_column)
...
def load_hbn_file_list(
    ...
    for _, row in meta.iterrows():
        ...
        age = float(row["Age_normalized_"]) 
        sex = int(row['Sex'])
       
        label_map_by_relpath[rel] = (age, sex)
