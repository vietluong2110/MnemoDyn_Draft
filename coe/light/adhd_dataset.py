...
class ADHD_Dataset(Dataset):
    def __init__(
        ...
        for rel_path in tqdm(self.file_list, desc="Preloading dtseries", unit="file"):
            ...
            try:
                ...
                print(f"[Unexpected Error] Could not load {full_path}: {e}")
                continue
    ...
    def __getitem__(self, idx: int):
        ...
        if self.label_column == 'age':
            label = label[0]
        return x, coeffs, label[1], self.time_step
...
class ADHDataModule:
    ...
    def setup(self):
        ...
        self.train_dataset = ADHD_Dataset(self.data_dir, self.train_files, self.label_map, self.time_step, 
                                    self.interpol_method, self.one_channel, self.subset, self.dim_D, 
                                    self.target_length, self.num_parcels, self.label_column)
        self.test_dataset = ADHD_Dataset(self.data_dir, self.test_files, self.label_map, self.time_step, 
                                    self.interpol_method, self.one_channel, self.subset, self.dim_D, 
                                    self.target_length, self.num_parcels, self.label_column)
...
def load_adhd_file_list(
    ...
    """
    if label_column not in {"gender", "dx"}:
        raise ValueError("label_column must be 'gender' or 'dx'")

    logger.info("Loading metadata from %s", metadata_path)
    meta = pd.read_csv(metadata_path)
    meta.columns = [c.strip() for c in meta.columns]

    required = {"path", "gender", "age", "dx"}
    missing = required - set(meta.columns)
    if missing:
        raise ValueError(f"Metadata CSV is missing columns: {missing}")

    # Convert numeric
    meta["_gender_int"] = meta["gender"].apply(_to_int_or_none)
    meta["_dx_int"] = meta["dx"].apply(_to_int_or_none)

    # Filter rows strictly
    if label_column == "gender":
        allowed = {0, 1}
        meta = meta[meta["_gender_int"].isin(allowed)]
        logger.info("Kept %d rows with gender ∈ {0,1}", len(meta))
    else:  # dx
        allowed = {0, 1, 2, 3}
        meta = meta[meta["_dx_int"].isin(allowed)]
        logger.info("Kept %d rows with dx ∈ {0,1,2,3}", len(meta))
    ...
    for _, row in meta.iterrows():
        ...
        if parcellate == 333 or parcellate == 424:

            rel = str(rel).replace(
                '450_parcellated.dtseries.nii',
                f'_{parcellate}_parcellated.dtseries.nii'
                )

        age = float(row["age"]) if pd.notna(row["age"]) else float("nan")

        if label_column == 'gender':
            g = int(row["_gender_int"])
            label_map_by_relpath[rel] = (age, g)
        else:
            d = int(row["_dx_int"])
            d_bin = 0 if d == 0 else 1
            label_map_by_relpath[rel] = (age, d_bin)
