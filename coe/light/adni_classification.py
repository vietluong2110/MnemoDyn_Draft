...
def parse_args():
	...
	parser.add_argument("--exp", default='ADNI_MCI_CN_cls_f1_oversampling', help="Adjusted in code: Experiment foler name")
...
def main():
	...
	all_files, label_map_str = load_adni_file_list(
		...
		'/mnt/sourav/fresh_ADNI/resting_state_fmri_info_sex.csv',
		'researchGroup'
	)
	...
	train_dataset = ADNI_Dataset(
		...
		dim_D          = 333,                  # must be a List[int] if subset=True
		target_length  = args.seq_length,       # e.g. 1200
		num_parcels    = 352,                   # make sure this matches whatever your .dtseries really has
...
# device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
	...
	# lit = LitHBCRModel.load_from_checkpoint(ckpt_path, map_location=device)
	# lit.eval()
	# foundation = lit.model.to(device)
	# for p in foundation.parameters():
	# 	p.requires_grad = False
	...
	# all_files, label_map = load_adni_file_list(
	# 	'/mnt/sourav/dtseries',
	# 	'/mnt/sourav/resting_state_fmri_info.csv',
	# )
	# logger.info("Total files loaded: %d", len(all_files))
