...
from compute_normalization_stats import StatsPrecomputer
...
class Normalizer_update:
    ...
    def transform_single(self, X):
        ...
        if method == 'raw':
            ...
            stats['global_iqr'] = [x if x != 0 else 1 for x in stats['global_iqr']]
...
class HCPParcellatedDataset(Dataset):
    def __init__(self, subject_dirs, file_name, time_step, interpol, D, subset=False
              , one_channel=None, normalizer=None, run_token="default"
              , patch_length=16
              , patch_start_index = 10
              , mask_ratio = 0.025
              , mask_value = 0.0
        ...
        self.valid_subjects = self.subject_dirs
    ...
    def _load_and_validate_data(self, file_path):
        ...
        # reps = int(np.ceil(1200 / data.shape[0]))
        # data = data.repeat(reps, 0)[:1200]
    ...
    def _rand_len(self, low, high):
        # torch.randint upper is exclusive; ensure >=1 span
        length = int(torch.randint(max(1, low), max(low+1, high+1), (1,)).item())
        return max(1, length)

    def _span_before(self, anchor, low_len, high_len):
        """Sample a [start,end) span with end<=anchor and length in [low_len, high_len]."""
        if anchor <= 1:
            return 0, 0  # empty if no room (you can also fallback to tiny span)
        max_len = min(high_len, anchor)
        length  = self._rand_len(min(low_len, max_len), max_len)
        start_hi = anchor - length
        if start_hi <= 0:
            return 0, length
        start = int(torch.randint(0, start_hi + 1, (1,)).item())
        return start, start + length
    def _span_after(self, anchor, n, low_len, high_len):
        """[start,end) fully after `anchor` up to n."""
        if anchor >= n-1:
            return n, n
        max_len = min(high_len, n - (anchor + 1))
        if max_len <= 0:
            return n, n
        length = self._rand_len(min(low_len, max_len), max_len)
        start_lo = anchor + 1
        ...
        if start_lo > start_hi:
            return n, n
        start = int(torch.randint(start_lo, start_hi+1, (1,)).item())
        return start, start + length

    def __getitem__(self, idx):
        data = self._load_single_subject(idx)   # [T, D]
        T, D = data.shape

        # ---- 1) observation block (as you specified) ----
        t0 = torch.randint(T//4, T//2, (1,)).item()
        # t1 = min(T, t0 + torch.randint(T//8, T//4, (1,)).item())
        t1 = t0 + 300
        r0 = torch.randint(D//4, D//2, (1,)).item()
        r1 = r0 + 300
        # r1 = min(D, r0 + torch.randint(D//8, D//4, (1,)).item())

        obs_mask = torch.zeros(T, D, dtype=torch.bool)
        obs_mask[t0:t1, r0:r1] = True

        # ---- 2) temporal target block: ONLY before t0, same ROI span (r0:r1) ----
        # choose a smaller length (e.g., 5–20% of T); tune as you like
        tb0, tb1 = self._span_before(
            anchor=t0,
            low_len=max(1, T//20),
            high_len=max(2, T//5)
        )
        
        temporal_mask = torch.zeros(T, D, dtype=torch.bool)
        temporal_mask[tb0:tb1, r0:r1] = True

        # ---- 3) spatial target block: ONLY before r0, same time span (t0:t1) ----
        rb0, rb1 = self._span_before(
            anchor=r0,
            low_len=max(1, D//20),
            high_len=max(2, D//5)
        )
        spatial_mask = torch.zeros(T, D, dtype=torch.bool)
        # if rb1 > rb0:
        spatial_mask[t0:t1, rb0:rb1] = True

        # Cross (γ) block: time BEFORE t0, ROI AFTER r1
        tg0, tg1 = self._span_before(
            anchor=t0,
            low_len=max(1, T//20),   # e.g., 5%..20% of T — tune as you like
            high_len=max(2, T//5)
        )
        rg0, rg1 = self._span_after(
            anchor=r1-1,             # after r1-1 means strictly to the right of [r0:r1)
            n=D,
            low_len=max(1, D//20),   # e.g., 5%..20% of D
            high_len=max(2, D//5)
        )

        cross_mask = torch.zeros(T, D, dtype=torch.bool)
        cross_mask[tg0:tg1, rg0:rg1] = True

        # ---- 4) build three independent masked views ----
        x_obs = torch.zeros_like(data)
        x_obs[obs_mask] = data[obs_mask]                   # keep only observation block

        x_temporal = torch.zeros_like(data)
        x_temporal[temporal_mask] = data[temporal_mask]    # keep only temporal block

        x_spatial = torch.zeros_like(data)
        x_spatial[spatial_mask] = data[spatial_mask]       # keep only spatial block

        x_cross    = torch.zeros_like(data)
        x_cross[cross_mask]      = data[cross_mask]

        # coeffs (same as before)
        batch_obs = x_obs.unsqueeze(0)
        batch_temp = x_temporal.unsqueeze(0)
        batch_spatial = x_spatial.unsqueeze(0)
        batch_cross = x_cross.unsqueeze(0)
        with torch.no_grad():
            if self.coeffs_method == "linear":
                coeffs_obs = torchcde.linear_interpolation_coeffs(batch_obs).squeeze(0)
                coeffs_temp = torchcde.linear_interpolation_coeffs(batch_temp).squeeze(0)
                coeffs_spatial = torchcde.linear_interpolation_coeffs(batch_spatial).squeeze(0)
                coeffs_cross = torchcde.linear_interpolation_coeffs(batch_cross).squeeze(0)

            else:
                coeffs_obs = torchcde.hermite_cubic_coefficients_with_backward_differences(batch_obs).squeeze(0)
                coeffs_temp = torchcde.hermite_cubic_coefficients_with_backward_differences(batch_temp).squeeze(0)
                coeffs_spatial = torchcde.hermite_cubic_coefficients_with_backward_differences(batch_spatial).squeeze(0)
                coeffs_cross = torchcde.hermite_cubic_coefficients_with_backward_differences(batch_cross).squeeze(0)

        return (
            x_obs, coeffs_obs,
            x_spatial, coeffs_spatial,
            x_temporal, coeffs_temp,
            x_cross, coeffs_cross,
            self.time_step
        )
...
class GordonHCPDataModule(pl.LightningDataModule):
    def __init__(self, **kwargs):
        ...
        # self.filename = "rfMRI_REST1_LR_Atlas_hp2000_clean_A424.dlabel_parcellated.dtseries.nii"
        ...
        # self.subject_dirs = self.subject_dirs[:36000]
    ...
    def setup(self, stage=None):
        ...
        pre = StatsPrecomputer(data_dir="/mnt/vhluong/ukbiobank/processed", cache_dir="./norm_cache")
        stats, self.train_dirs, self.val_dirs = pre.reload_cache()
    ...
    def train_dataloader(self):
        return DataLoader(self.train_dataset, batch_size=self.args.train_bs, num_workers=8)

    def val_dataloader(self):
        return DataLoader(self.val_dataset, batch_size=self.args.valid_bs, num_workers=8)

    def test_dataloader(self):
        return DataLoader(self.test_dataset, batch_size=self.args.test_bs, num_workers=2)
...
class LitORionModelOptimized(pl.LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        # batch comes from MaskedCDEDataset:
        # x_obs_aug: [B, T, Din or Din+1]  (masked signal, maybe with mask channel)
        # coeff:     torchcde coeffs built from x_obs_aug
        # x_target:  [B, T, Dout]          (original unmasked series)
        # is_masked_t: [B, T] bool         (True where we hid data)
        # time:      scalar or [B]
        x_obs_aug, coeff_obs , x_spatial, coeff_spatial,x_temp, coeff_temp, x_cross, coeff_cross, time = batch
        if batch_idx % 25 == 0:
            torch.cuda.empty_cache()
        
        # Forward
        with torch.cuda.amp.autocast(enabled=self.trainer.precision == '16-mixed'):
            obs_out = self(x_obs_aug, coeff_obs, time)   # <== feed the MASKED observation
            spatial_out = self(x_spatial, coeff_spatial, time)
            temp_out = self(x_temp, coeff_temp, time)
            cross_out = self(x_cross, coeff_cross, time)
         
        # ---- Masked losses (compute ONLY where masked) ----
        # MSE
        mse_spatial = self.mse_loss(obs_out, spatial_out)
        mse_temp = self.mse_loss(obs_out, temp_out)
        mse_cross = self.mse_loss(obs_out, cross_out)
        ...
        mse = (mse_spatial + mse_cross + mse_temp) / 3 
        loss = mse
...
def main():
    ...
    checkpoint_callback = pl.callbacks.ModelCheckpoint(
        ...
        mode="min",
        save_top_k=2,
        filename="model-{epoch:02d}-{val_mse:.4f}",
        verbose=True,
    )
