...
class Normalizer_update:
    ...
    def transform_single(self, X):
        ...
        if method == 'raw':
            ...
            stats['global_iqr'] = [x if x != 0 else 30000 for x in stats['global_iqr']]
...
class HCPParcellatedDataset(Dataset):
    ...
    def _load_single_subject(self, idx):
        ...
        data_tensor = torch.from_numpy(arr).contiguous().clone()  # break NumPy storage sharing
        ...
        return data_tensor[:490, :]

    def __len__(self):
        return len(self.valid_subjects)

    # def _build_mask(self, T):
    # 	# Priority: contiguous patch if patch_length>0, else random-k by ratio.
    # 	mask = torch.zeros(T, dtype=torch.bool)
    # 	if self.patch_length and self.patch_length > 0:
    # 		L = min(self.patch_length, T)
    # 		if self.patch_start_index is None:
    # 			start = torch.randint(0, T - L + 1, (1,)).item()
    # 		else:
    # 			start = max(0, min(self.patch_start_index, T - L))
    # 		mask[start:start + L] = True
    # 	elif self.mask_ratio and self.mask_ratio > 0:
    # 		K = max(1, int(round(T * self.mask_ratio)))
    # 		idx = torch.randperm(T)[:K]
    # 		mask[idx] = True
    # 	return mask

    def _build_mask_TD(self, T: int, D: int):
        """

        Returns a boolean mask of shape [T, D].
        For each feature (column) independently:
        - place K non-overlapping windows
        - each window has length L
        - K is derived from mask_ratio as 'window COUNT' (your desired semantics)
        """
        mask = torch.zeros(T, D, dtype=torch.bool)
        if not (getattr(self, "mask_ratio", None) and self.mask_ratio > 0):
            return mask  # no masking

        L = max(1, int(getattr(self, "patch_length", 3)))         # window length
        K_req = max(1, int(round(T * self.mask_ratio)))                  # windows per feature
        K_cap = max(1, T // L)                                           # max disjoint windows possible
        K = min(K_req, K_cap)

        max_start = max(1, T - L + 1)                                    # valid window starts: [0..T-L]

        # For each feature independently, sample K non-overlapping starts
        for d in range(D):
            starts_space = torch.randperm(max_start).tolist()
            chosen = []
            for s in starts_space:
                # ensure no overlap with chosen windows in this column
                if all(not (s < c + L and c < s + L) for c in chosen):
                    chosen.append(s)
                    if len(chosen) == K:
                        break
            for s in chosen:
    ...
    def __getitem__(self, idx):
        ...
        if data.dim() != 2:
            raise ValueError(f"Expected [T, D], got {tuple(data.shape)}")
        T, D = data.shape
        # pdb.set_trace()
        # ---- 2) Build time mask ----
        is_masked_t = self._build_mask_TD(T, D)              # [T] boolean (True where we hide)

        # ---- 3) Create masked observation signal (keep shape the same) ----
        data_obs = data.clone()
        # if is_masked_t.any():
            # Replace ALL feature dims at masked timesteps
        data_obs[is_masked_t] = self.mask_value

        # Optional but recommended for CDEs: add an indicator channel
        # if self.add_mask_channel:
        #     m = is_masked_t.to(data.dtype).unsqueeze(-1)   # [T, 1]
        #     data_obs_aug = torch.cat([data_obs, m], dim=-1)  # [T, D+1]
        # else:
        data_obs_aug = data_obs                        # [T, D]

        # ---- 4) Compute interpolation coefficients on the OBSERVATION signal ----
        data_batch = data_obs_aug.unsqueeze(0)             # [1, T, D’]
        with torch.no_grad():
        ...
        # coeffs shape is whatever torchcde returns for your chosen scheme
        # ---- 5) Return everything needed for MAE-style training ----
        # - data_obs_aug: what you feed to the model (masked, plus mask-channel)
        # - coeffs: interpolation coeffs computed on that masked signal
        # - data (original): target for reconstruction loss
        # - is_masked_t: to restrict loss to masked timesteps
        # - time_step: unchanged
        return data_obs_aug, coeffs, data, is_masked_t, self.time_step
...
class GordonHCPDataModule(pl.LightningDataModule):
    def __init__(self, **kwargs):
        ...
        # self.filename = "rfMRI_REST1_LR_Atlas_hp2000_clean_Gordon333_FreesurferSubcortical.32k_fs_LR.dlabel_parcellated.dtseries.nii"
...
class LitORionModelOptimized(pl.LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        # time:      scalar or [B]
        x_obs_aug, coeff, x_target, is_masked_t, time = batch
        if batch_idx % 25 == 0:
            torch.cuda.empty_cache()
        
        mask = is_masked_t
        def count_mask_windows(mask: torch.Tensor):
            """
            Count number of contiguous masked windows in each column.

            Args:
                mask (torch.Tensor): shape [B, T, D], boolean tensor

            Returns:
                torch.Tensor: shape [B, D], number of windows per column
            """
            B, T, D = mask.shape
            # Shift mask by 1 along time axis to detect starts of windows
            pad = torch.zeros((B, 1, D), dtype=mask.dtype, device=mask.device)
            shifted = torch.cat([pad, mask[:, :-1, :]], dim=1)

            # Window starts where current==True and previous==False
            window_starts = mask & ~shifted

            # Count starts per column
            return window_starts.sum(dim=1)
        counts = count_mask_windows(mask)  # shape [8, 450]

        # Build broadcastable mask over features
        # If your model outputs the same feature dim as x_target:
        # mask = is_masked_t.unsqueeze(-1).expand_as(x_target)   # [B, T, D]

        # If you sometimes set patch_length=0 (no masking), avoid empty loss:
        # if not mask.any():
        # 	# fall back to full loss (rare; e.g., when you turn masking off)
        # 	mask = torch.ones_like(mask, dtype=torch.bool)

        # Forward
        with torch.cuda.amp.autocast(enabled=self.trainer.precision == '16-mixed'):
            out = self(x_obs_aug, coeff, time)   # <== feed the MASKED observation
        ...
        mse = self.mse_loss(x_pred, x_target)
        if self.args.use_mRLoss:
            mse = mse + mRloss

        # MAE
     
        mae = self.mae_loss(x_pred, x_target)
        huber = self.huber_loss(x_pred, x_target)
