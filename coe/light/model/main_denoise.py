...
class HCPParcellatedDataset(Dataset):
	def __init__(self, subject_dirs, file_name, time_step, interpol, D, subset=False, one_channel=None, normalizer=None, run_token="default", add_noise=True, noise_scale=0.1):
	...
	def __getitem__(self, idx):
		...
		if self.add_noise:
			noised_data = data + self.noise_scale * torch.randn(data.shape)
		# Compute interpolation coefficients
		noised_data_batch = noised_data.unsqueeze(0)
		with torch.no_grad():
			if self.coeffs_method == 'linear':
				coeffs = torchcde.linear_interpolation_coeffs(noised_data_batch).squeeze(0)
			else:
				coeffs = torchcde.hermite_cubic_coefficients_with_backward_differences(noised_data_batch).squeeze(0)

		return data, noised_data, coeffs, self.time_step
