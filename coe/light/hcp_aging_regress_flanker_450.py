...
def parse_args():
	...
	parser.add_argument("--exp", default='HCP_Aging_Regression_Flanker_450_braindynamo_mask_jepa', help="Adjusted in code: Experiment foler name")
...
def main():
	...
	all_files, label_map = load_hcp_aging_parcellated_list(
		'/mnt/vhluong/HCPA_with_flanker_scores.csv',
		'/mnt/sourav/HCPAging/dtseries',
		label_column='nih_flanker_computed'
	)
	# pdb.set_trace()
	...
	for epoch in range(args.epoch):
		...
		if len(np.unique(all_targets)) > 1:
			pearson_cor, _ = pearsonr(all_preds, all_targets)
		else:
			pearson_cor = float('nan')
		pearson_corr_list.append(pearson_cor)
		@torch.no_grad()
		...
		r2_score_torch_(all_targets, all_preds)
		logger.info("Epoch %d VALID loss=%.4f, Pearson r=%.4f", epoch, epoch_val_loss, pearson_cor)
