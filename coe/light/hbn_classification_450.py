...
import os, glob
from hbn_dataset import HBN_Dataset, load_hbn_file_list
...
def parse_args():
    ...
    parser.add_argument("--exp", default='HBN_Sex_Classification_450_10_Times', help="Adjusted in code: Experiment foler name")
...
def main():
    ...
    labels = [label_map_str[f][1] for f in all_files]

    counts = Counter(labels)
    print(counts) 

    files_0 = [f for f in all_files if label_map_str[f][1] == 0]
    files_1 = [f for f in all_files if label_map_str[f][1] == 1]

    # Upsample 1s by duplicating until same size as 0s
    random.seed(42)
    files_1_upsampled = random.choices(files_1, k=len(files_0))

    balanced_files = files_0 + files_1_upsampled
    balanced_labels = [label_map_str[f][1] for f in balanced_files]

    print("Balanced distribution:", Counter(balanced_labels))
    


    # ─── Split train/test ───────────────────────────────────────────────────────────
    train_files, test_files, train_labels, test_labels = train_test_split(balanced_files, balanced_labels, test_size=0.2, random_state=42)
    ...
    train_dataset = HBN_Dataset(
        data_dir       = '/mnt/sourav/HBN_dtseries/',
    ...
    checkpoint = torch.load("/workspace/high_dim_BCR/coe/light/Result/HCP_Aging_Classification_Few_Shot_Model/Results/8/model_30.pt", map_location=device)
    ...
    for epoch in range(args.epoch):
        ...
        with torch.no_grad():
            for x, coeffs, y_true, time in tqdm(test_dl, leave=False):
                ...
                save_dir = "reconstruction_outputs_hbn"
