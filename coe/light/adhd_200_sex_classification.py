...
def main():
    ...
    all_files, label_map_str = load_adhd_file_list(
        '/mnt/sourav/ADHD200_dtseries/',
        '/mnt/vhluong/adhd_paths_labels_mapped.csv',
        'gender', 
        450
    )
    ...
    neg_files = [f for f in all_files if label_map_str[f][1] == 0.0]
    pos_files = [f for f in all_files if label_map_str[f][1] == 1.0]

    # downsample positives to match number of negatives
    random.seed(42)  # for reproducibility
    pos_files_sampled = random.sample(pos_files, len(neg_files))

    # new balanced file list
    balanced_files = neg_files + pos_files_sampled

    # corresponding labels
    balanced_labels = [label_map_str[f][1] for f in balanced_files]

    print("Before:", len(neg_files), "negatives,", len(pos_files), "positives")
    print("After:", len(balanced_files), "total balanced samples")


    # ─── Split train/test ───────────────────────────────────────────────────────────
    train_files, test_files, train_labels, test_labels = train_test_split(balanced_files, balanced_labels, test_size=0.20, random_state=42)
    ...
    train_dataset = ADHD_Dataset(
        data_dir       = '/mnt/sourav/ADHD200_dtseries/',
        ...
        label_column   = 'gender'        # <-- this makes __getitem__ return grp_idx
    )
    logger.info("Train dataset size: %d", len(train_dataset))
    test_dataset = ADHD_Dataset(
        data_dir       = '/mnt/sourav/ADHD200_dtseries/',
