...
import os, glob
from demo_data import Demo_Dataset, load_demo_file_list
...
def main():
    ...
    all_files, label_map_str = load_demo_file_list(
        '/mnt/vhluong/ds005747-download/dtseries',
        '/mnt/vhluong/ds005747-download/participants.tsv',
        'gender',
        450
    )
    ...
    # ─── Split train/test ───────────────────────────────────────────────────────────
    train_files, test_files, train_labels, test_labels = train_test_split(all_files, labels, test_size=0.4, random_state=42)
    ...
    train_dataset = Demo_Dataset(
        data_dir       = '/mnt/vhluong/ds005747-download/dtseries',
    ...
    recon_save_dir = os.path.join(args.exp, "reconstruction_outputs_ds005747")
    ...
    for epoch in range(args.epoch):
        ...
        test_preds = []
        with torch.no_grad():
            for batch_idx, (x, coeffs, y_true, time) in enumerate(tqdm(test_dl, leave=False)):
                x, coeffs, y_true, time = x.to(device).float(), coeffs.to(device).float(), y_true.to(device), time.to(device).float()
                # U is the reconstruction output from the frozen foundation model.
                U = foundation(x, coeffs, time)

                # Save reconstruction output and corresponding input for this batch.
                U_np = U.detach().cpu().numpy()
                x_np = x.detach().cpu().numpy()
                np.save(
                    os.path.join(recon_save_dir, f"epoch_{epoch:03d}_batch_{batch_idx:04d}_U_recon.npy"),
                    U_np,
                )
                np.save(
                    os.path.join(recon_save_dir, f"epoch_{epoch:03d}_batch_{batch_idx:04d}_x_input.npy"),
                    x_np,
                )
